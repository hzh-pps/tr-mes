...
let importDialog = ref<boolean>(false);
let importDialog2 = ref<boolean>(false);
// 工单导入
function showImport() {
...
async function addDetailSucces2() {
  for (const [index, item] of workOrderData.value.entries()) {
    if (item.workorder_type === "") {
      console.log(`工单类型为空，索引：${index}`, item);
      setSnackbar("black", "第" + (index + 1) + "行工单类型不能为空");
      return;
    }
  }
  workOrderData.value = workOrderData.value.map((item: any) => {
    if (Array.isArray(item.workorder_type)) {
      item.workorder_type = item.workorder_type.join(",");
    }
    return item;
  });
  if (workOrderData.value.length < 2) {
    workOrderData.value.push({ ...workOrderData.value[0] });
  }
  const data: any = await useHttp(
    "/MesWorkOrderDetail/M109AddWorkOrderDetailsByExcel",
    "post",
    workOrderData.value
  );
  if (data.code === 200) {
...
function subData(itemToDelete: any) {
  workOrderData.value = workOrderData.value.filter(
      ...
      <v-card>
        ...
        <v-card-text class="mt-4">
          <v-row v-for="(item, index) in workOrderData">
            ...
            <v-col>
              <v-icon class="text-blue" @click="subData(item)">
                fa-solid fa-minus
              </v-icon>
            </v-col>
          </v-row>
